...
void DaySetFromJsonString(char *root, DaySet *daySet) {
	...
	for (int i = 0; i < MAX_DAYS; ++i) {
		...
		js_read_int(&object, "num",&daySet->days[i].num );
		js_read_int(&object, "count",&daySet->days[i].count );
		js_read_array(&objday, &jslines, "liness");
		for (int j = 0; j < MAX_LINES; ++j) {
			if(js_read_array_object(jslines, j, objline)!=JsonSuccess) break;
			int min,hour;
			js_read_int(&objday, "npk",&daySet->days[i].lines[j].npk );
			js_read_int(&objday, "hour",&hour );
			js_read_int(&objday, "min",&min );
