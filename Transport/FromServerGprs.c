...
void FromServerGPRSLoop(void) {
	...
	if (err != 0) {
		Debug_Message(LOG_ERROR,
				"GPRS Нет соединения с сервером по основному каналу");
		veryBad(buffer,socket,GPRSFromServerQueue);
		return;
	}
	...
	for (;;) {
		...
		if (len < 0) {
			Debug_Message(LOG_ERROR, "GPRS Ошибка чтения ");
			veryBad(buffer,socket,GPRSFromServerQueue);
			return;
		}
		if (len == 0) continue;
		if (buffer[len - 1] != '\n'){
			Debug_Message(LOG_ERROR, "GPRS Неверное завершение строки %s",buffer);
			veryBad(buffer,socket,GPRSFromServerQueue);
			return;
		}
		buffer[len - 1] = 0;
		...
		osMessageQueuePut(GPRSFromServerQueue, &msg, 0, 0);
		if (osMessageQueueGet(GPRSToServerQueue, &msg, NULL, toque) != osOK) {
			veryBad(buffer,socket,GPRSFromServerQueue);
			return;
		}
