...
int RAM_VALID(LBA_t sector, UINT count){
	...
	if ((sector+count)>RAM_COUNT_SECTORS) return RES_ERROR;
	return RES_OK;
}
int RAM_disk_status(){
	if (RAM_BUFFER!=NULL) return RES_OK;
	return RES_ERROR;
}
int MMC_disk_status(){
	return RES_ERROR;
...
int RAM_disk_read(BYTE * buff, LBA_t sector, UINT count){
	if (RAM_VALID(sector, count)!=RES_OK) return RES_ERROR;
	memcpy(buff,RAM_BUFFER+sector*RAM_SIZE_SECTOR,RAM_SIZE_SECTOR*count);
	return RES_OK;
}
...
int RAM_disk_write(const BYTE * buff, LBA_t sector, UINT count){
	if (RAM_VALID(sector, count)!=RES_OK) return RES_ERROR;
	memcpy(RAM_BUFFER+sector*RAM_SIZE_SECTOR,buff,RAM_SIZE_SECTOR*count);
	return RES_OK;
}
