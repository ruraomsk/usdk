...
void _error_print(modbus_t *ctx, const char *context) {
	if (ctx->debug) {
		...
		if (context != NULL) {
			Debug_Message(LOG_ERROR, "ERROR %s : %s", modbus_strerror(errno),
					context);
		} else {
...
int modbus_send_raw_request(modbus_t *ctx, const uint8_t *raw_req,
		int raw_req_length) {
	sft_t sft;
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type) {
	...
	if (msg_type == MSG_INDICATION) {
		 ...
		 * received */
		if (ctx->indication_timeout.tv_sec == 0
...
static int response_exception(modbus_t *ctx, sft_t *sft, int exception_code,
		uint8_t *rsp, unsigned int to_flush, const char *template, ...) {
	int rsp_length;
...
int modbus_reply(modbus_t *ctx, const uint8_t *req, int req_length,
	...
	case MODBUS_FC_READ_DISCRETE_INPUTS: {
		...
		int start_bits =
				is_input ?
						mb_mapping->start_input_bits : mb_mapping->start_bits;
		int nb_bits =
				is_input ? mb_mapping->nb_input_bits : mb_mapping->nb_bits;
		uint8_t *tab_bits =
				is_input ? mb_mapping->tab_input_bits : mb_mapping->tab_bits;
		...
		if (nb < 1 || MODBUS_MAX_READ_BITS < nb) {
			...
			rsp_length = response_io_status(tab_bits, mapping_address, nb, rsp,
					rsp_length);
		}
	}
		break;
	...
	case MODBUS_FC_READ_INPUT_REGISTERS: {
		...
		const char *const name =
				is_input ? "read_input_registers" : "read_registers";
		int nb = (req[offset + 3] << 8) + req[offset + 4];
		/* The mapping can be shifted to reduce memory consumption and it
		 doesn't always start at address zero. */
	...
	case MODBUS_FC_WRITE_SINGLE_REGISTER: {
		...
		if (mapping_address < 0
				|| mapping_address >= mb_mapping->nb_registers) {
			rsp_length = response_exception(ctx, &sft,
					MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp, FALSE,
	...
	case MODBUS_FC_WRITE_MULTIPLE_COILS: {
		...
		if (nb < 1 || MODBUS_MAX_WRITE_BITS < nb || nb_bits * 8 < nb) {
			...
			/* 6 = byte count */
			modbus_set_bits_from_bytes(mb_mapping->tab_bits, mapping_address,
					nb, &req[offset + 6]);
	...
	case MODBUS_FC_WRITE_AND_READ_REGISTERS: {
		...
		if (nb_write < 1 || MODBUS_MAX_WR_WRITE_REGISTERS < nb_write || nb < 1
				|| MODBUS_MAX_WR_READ_REGISTERS < nb
			...
			rsp_length =
					response_exception(ctx, &sft,
							...
							mapping_address_write < 0 ?
									address_write : address_write + nb_write);
		} else {
			int i, j;
			rsp_length = ctx->backend->build_response_basis(&sft, rsp);
			rsp[rsp_length++] = nb << 1;
...
int modbus_read_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest) {
	...
	if (nb > MODBUS_MAX_READ_BITS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR, "ERROR Too many bits requested (%d > %d)",
	...
	if (nb > MODBUS_MAX_READ_BITS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR,
					"ERROR Too many discrete inputs requested (%d > %d)", nb,
					MODBUS_MAX_READ_BITS);
		}
		errno = EMBMDATA;
		return -1;
...
static int read_registers(modbus_t *ctx, int function, int addr, int nb,
	...
	if (nb > MODBUS_MAX_READ_REGISTERS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR,
					"ERROR Too many registers requested (%d > %d)", nb,
					MODBUS_MAX_READ_REGISTERS);
		}
		errno = EMBMDATA;
		return -1;
...
int modbus_read_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest) {
	...
	status = read_registers(ctx, MODBUS_FC_READ_HOLDING_REGISTERS, addr, nb,
			dest);
	return status;
}
...
int modbus_read_input_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest) {
	...
	if (nb > MODBUS_MAX_READ_REGISTERS) {
			...
			Debug_Message(LOG_ERROR,
					"ERROR Too many input registers requested (%d > %d)", nb,
					MODBUS_MAX_READ_REGISTERS);
		errno = EMBMDATA;
		return -1;
	}

	status = read_registers(ctx, MODBUS_FC_READ_INPUT_REGISTERS, addr, nb,
			dest);
...
static int write_single(modbus_t *ctx, int function, int addr,
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src) {
	...
	if (nb > MODBUS_MAX_WRITE_BITS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR, "ERROR Writing too many bits (%d > %d)",
...
int modbus_write_registers(modbus_t *ctx, int addr, int nb, const uint16_t *src) {
	...
	if (nb > MODBUS_MAX_WRITE_REGISTERS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR,
					"ERROR Trying to write to too many registers (%d > %d)", nb,
...

{
	...
	if (write_nb > MODBUS_MAX_WR_WRITE_REGISTERS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR,
					"ERROR Too many registers to write (%d > %d)", write_nb,
	...
	if (read_nb > MODBUS_MAX_WR_READ_REGISTERS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR,
					"ERROR Too many registers requested (%d > %d)", read_nb,
					MODBUS_MAX_WR_READ_REGISTERS);
		}
		errno = EMBMDATA;
		return -1;
...
modbus_mapping_t* modbus_mapping_new_start_address(unsigned int start_bits,
		unsigned int nb_bits, unsigned int start_input_bits,
		unsigned int nb_input_bits, unsigned int start_registers,
		unsigned int nb_registers, unsigned int start_input_registers,
...
modbus_mapping_t* modbus_mapping_new(int nb_bits, int nb_input_bits,
	...
	return modbus_mapping_new_start_address(0, nb_bits, 0, nb_input_bits, 0,
			nb_registers, 0, nb_input_registers);
}
