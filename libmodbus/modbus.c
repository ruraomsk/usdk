...
void _error_print(modbus_t *ctx, const char *context) {
	if (ctx->debug) {
		...
		if (context != NULL) {
			Debug_Message(LOG_ERROR, "ERROR %s : %s", modbus_strerror(errno), context);
		} else {
			Debug_Message(LOG_ERROR, "ERROR %s ", modbus_strerror(errno));
		}
	}
...
static int compute_data_length_after_meta(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type) {
	...
	if (msg_type == MSG_INDICATION) {
		...
		/* MSG_CONFIRMATION */
		if (function <= MODBUS_FC_READ_INPUT_REGISTERS || function == MODBUS_FC_REPORT_SLAVE_ID
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type) {
	...
	if (ctx->debug) {
		if (msg_type == MSG_INDICATION) {
			Debug_Message(LOG_INFO, "Waiting for an indication...");
		} else {
			Debug_Message(LOG_INFO, "Waiting for a confirmation...");
		}
	}
...
static int check_confirmation(modbus_t *ctx, uint8_t *req, uint8_t *rsp, int rsp_length) {
	...
	if ((rsp_length == rsp_length_computed || rsp_length_computed == MSG_LENGTH_UNDEFINED) && function < 0x80) {
		...
		if (function != req[offset]) {
			if (ctx->debug) {
				Debug_Message(LOG_INFO, "Received function not corresponding to the request (0x%X != 0x%X)", function,
						req[offset]);
			}
			if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) {
				_sleep_response_timeout(ctx);
		...
		if (req_nb_value == rsp_nb_value) {
			...
			if (ctx->debug) {
				Debug_Message(LOG_INFO, "Quantity not corresponding to the request (%d != %d)", rsp_nb_value,
						req_nb_value);
			}
		...
		if (ctx->debug) {
			Debug_Message(LOG_INFO, "Message length not corresponding to the computed length (%d != %d)", rsp_length,
					rsp_length_computed);
		}
		if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) {
			_sleep_response_timeout(ctx);
...
static int response_io_status(uint8_t *tab_io_status, int address, int nb, uint8_t *rsp, int offset) {
	...
	if (shift != 0) rsp[offset++] = one_byte;

	return offset;
}

/* Build the exception response */
static int response_exception(modbus_t *ctx, sft_t *sft, int exception_code, uint8_t *rsp, unsigned int to_flush,
		const char *template, ...) {
	int rsp_length;
...
int modbus_reply(modbus_t *ctx, const uint8_t *req, int req_length, modbus_mapping_t *mb_mapping) {
	...
	case MODBUS_FC_READ_EXCEPTION_STATUS:
		if (ctx->debug) {
			Debug_Message(LOG_ERROR, "FIXME Not implemented\n");
		}
		errno = ENOPROTOOPT;
		return -1;
		break;
	...
	case MODBUS_FC_WRITE_AND_READ_REGISTERS: {
		...
		if (nb_write < 1 || MODBUS_MAX_WR_WRITE_REGISTERS < nb_write || nb < 1 || MODBUS_MAX_WR_READ_REGISTERS < nb
				|| nb_write_bytes != nb_write * 2) {
			rsp_length = response_exception(ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, TRUE,
					"Illegal nb of values (W%d, R%d) in write_and_read_registers (max W%d, R%d)\n", nb_write, nb,
					MODBUS_MAX_WR_WRITE_REGISTERS,
					MODBUS_MAX_WR_READ_REGISTERS);
		} else if (mapping_address < 0 || (mapping_address + nb) > mb_mapping->nb_registers || mapping_address < 0
	...
	return (ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_RTU && slave == MODBUS_BROADCAST_ADDRESS) ?
			0 : send_msg(ctx, rsp, rsp_length);
}
...
int modbus_read_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest) {
	...
	if (nb > MODBUS_MAX_READ_BITS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR, "ERROR Too many bits requested (%d > %d)", nb, MODBUS_MAX_READ_BITS);
		}
		errno = EMBMDATA;
		return -1;
	}
...
int modbus_read_input_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest) {
	...
	if (nb > MODBUS_MAX_READ_BITS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR, "ERROR Too many discrete inputs requested (%d > %d)", nb,
			MODBUS_MAX_READ_BITS);
		}
		errno = EMBMDATA;
		return -1;
...
static int read_registers(modbus_t *ctx, int function, int addr, int nb, uint16_t *dest) {
	...
	if (nb > MODBUS_MAX_READ_REGISTERS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR, "ERROR Too many registers requested (%d > %d)", nb,
			MODBUS_MAX_READ_REGISTERS);
		}
		errno = EMBMDATA;
		return -1;
...
int modbus_read_input_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest) {
	...
	if (nb > MODBUS_MAX_READ_REGISTERS) {
		...
		Debug_Message(LOG_ERROR, "ERROR Too many input registers requested (%d > %d)", nb,
		MODBUS_MAX_READ_REGISTERS);
		errno = EMBMDATA;
		return -1;
	}
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src) {
	...
	if (nb > MODBUS_MAX_WRITE_BITS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR, "ERROR Writing too many bits (%d > %d)", nb, MODBUS_MAX_WRITE_BITS);
		}
		errno = EMBMDATA;
		return -1;
	}
	...
	MODBUS_FC_WRITE_MULTIPLE_COILS, addr, nb, req);
...
int modbus_write_registers(modbus_t *ctx, int addr, int nb, const uint16_t *src) {
	...
	if (nb > MODBUS_MAX_WRITE_REGISTERS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR, "ERROR Trying to write to too many registers (%d > %d)", nb,
	...
	req_length = ctx->backend->build_request_basis(ctx,
	MODBUS_FC_WRITE_MULTIPLE_REGISTERS, addr, nb, req);
	byte_count = nb * 2;
	req[req_length++] = byte_count;
...
int modbus_mask_write_register(modbus_t *ctx, int addr, uint16_t and_mask, uint16_t or_mask) {
	...
	req_length = ctx->backend->build_request_basis(ctx,
	MODBUS_FC_MASK_WRITE_REGISTER, addr, 0, req);
...
int modbus_write_and_read_registers(modbus_t *ctx, int write_addr, int write_nb, const uint16_t *src, int read_addr,
...

{
	...
	if (write_nb > MODBUS_MAX_WR_WRITE_REGISTERS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR, "ERROR Too many registers to write (%d > %d)", write_nb,
			MODBUS_MAX_WR_WRITE_REGISTERS);
		}
		errno = EMBMDATA;
		return -1;
	...
	if (read_nb > MODBUS_MAX_WR_READ_REGISTERS) {
		if (ctx->debug) {
			Debug_Message(LOG_ERROR, "ERROR Too many registers requested (%d > %d)", read_nb,
			MODBUS_MAX_WR_READ_REGISTERS);
		}
		errno = EMBMDATA;
		return -1;
	...
	req_length = ctx->backend->build_request_basis(ctx,
	MODBUS_FC_WRITE_AND_READ_REGISTERS, read_addr, read_nb, req);
...
int modbus_report_slave_id(modbus_t *ctx, int max_dest, uint8_t *dest) {
	...
	req_length = ctx->backend->build_request_basis(ctx,
	MODBUS_FC_REPORT_SLAVE_ID, 0, 0, req);
...
modbus_mapping_t* modbus_mapping_new_start_address(unsigned int start_bits, unsigned int nb_bits,
		unsigned int start_input_bits, unsigned int nb_input_bits, unsigned int start_registers,
		unsigned int nb_registers, unsigned int start_input_registers, unsigned int nb_input_registers) {
	...
	if (nb_input_registers == 0) {
		...
		mb_mapping->tab_input_registers = (uint16_t*) pvPortMalloc(nb_input_registers * sizeof(uint16_t));
...
size_t strlcpy(char *dest, const char *src, size_t dest_size) {
	...
	if (n == 0) {
		if (dest_size != 0) *d = '\0'; /* NUL-terminate dest */
		while (*s++)
			;
	}
